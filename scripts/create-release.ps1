...
$tag  = "v0.7.0"
$name = "v0.7.0 · 数据目录绑定 + 首启环境检测 + 升级数据保护"
$body = "## 新增`n- 数据目录绑定：所有运行时数据（数据库 / 浏览器登录态 / 截图缓存）统一落在「安装目录\data」，不再散落 C 盘；旧数据首次启动自动迁移（复制不删除，幂等）`n- 首启环境检测重做：分组检测卡 + 环形进度 + 扫描光束，处理器/内存/显卡/WebView2 全部真实探测，数据绑定卡展示实际落盘路径并支持一键打开`n- 升级数据保护（三层）：应用内升级前自动备份数据目录；NSIS 安装钩子卸载前备份、装完回填；启动时检测缺库自动从升级备份恢复——模型 / 语音 / 记忆等设置升级后原样保留"
