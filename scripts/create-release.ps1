...
$tag  = "v0.2.0"
$name = "v0.2.0 · 计划确认 + 消息中心"
$body = "## 新增`n- 计划确认机制：多步任务先回答情况（可边答边调工具），再弹计划卡等你确认后才执行——聊天不再等任务拆解，简单问答零额外调用`n- 消息中心：待审批 / IM 消息 / 定时提醒三合一面板，待审批数徽标提醒，审批行内直接同意/拒绝`n- 计划确认卡支持 IM 批准：手机上回复「允许/拒绝」即可放行，复用通知升级链`n## 变更`n- 移除预拆解：原「疑似多步任务先跑一次模型拆解」改为模型自行提交计划等确认，首响应更快更省 token"
