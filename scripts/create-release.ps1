...
$tag  = "v0.1.7"
$name = "v0.1.7 · 游戏自动化（局面缓存/宏序列）"
$body = "## 新增（回合制/半即时游戏通用：金铲铲、云顶、崩铁等）`n- region_ocr 区域识别：只 OCR 棋盘/商店/血条等固定小区域，返回词的绝对坐标可直接点击，比全屏快且噪音少`n- board_diff 局面缓存：按 key 缓存多区域快照（本地 JSON），下回合自动增量 diff 只返回变化区域，决策不再全量重扫`n- macro 宏序列：按键/点击/等待一次调用批量执行（≤30 步）——「D 牌 N 次」、战斗连招、种植占格一次搞定，点击沿用拟人化注入`n- 系统提示词新增【游戏自动化】战术段：感知提速 + 动作合并 + 崩铁小地图罗盘跑图预案，形成「感知→决策→合并动作→确认」稳定循环`n## 优化`n- region_ocr / board_diff 计入观察类工具，不触发循环检测误判`n- 局面快照存 %TEMP%\baize-board\，随自维护自动清理"
