...
fn run_python(request: &Value, progress: Option<&Progress>) -> Result<Value, String> {
    ...
    let mut child = crate::tools::python_program()
        ...
        .map_err(|e| {
            format!(
                "启动 Python 失败（安装包内置 pyoffice 缺失且系统未装 Python；可尝试 pip install pdfplumber python-docx openpyxl python-pptx）: {e}"
            )
        })?;
...
pub fn deps_report() -> Value {
    ...
    if let Ok(out) = crate::tools::python_program().args(["-c", DEPS_PROBE]).output() {
        ...
        } else if let Ok(o) = crate::tools::python_program().arg("--version").output() {
